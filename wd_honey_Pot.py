...
class HoneyPotPlugin(Plugin):
    ...
    def log(self, message):
        ...
        masked_message = message.replace(self.generate_random_mac_address(),
    ...
    def add_fake_ap(self):
        ...
        self.log(f"Added fake access point: {fake_ap_info['essid']} ({fake_ap_info['addr']})")
    ...
    def remove_fake_ap(self, fake_ap_info):
        ...
        if fake_ap_info in self.honey_pot_aps:
            ...
            self.log(f"Removed fake access point: {fake_ap_info['essid']} ({fake_ap_info['addr']})")
